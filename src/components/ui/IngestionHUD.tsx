...
import { Loader2, CheckCircle2, AlertCircle, FileText, X, Brain, Wind } from 'lucide-react';
...
export function IngestionHUD() {
    ...
    return (
        <DraggableHUD id="ingestion-hud" title="Ingestión" defaultPosition={{ x: window.innerWidth - 380, y: window.innerHeight - 300 }}>
            <div className="flex flex-col gap-3 max-w-sm w-full">
                {jobs.map(job => (
                    <div
                        key={job.id}
                        className="group bg-white/40 backdrop-blur-xl border border-white/40 rounded-2xl p-4 shadow-[0_8px_32px_rgba(0,0,0,0.1)] transition-all duration-500 hover:scale-[1.02] hover:bg-white/60 animate-in slide-in-from-bottom-5 fade-in"
                    >
